...
func apply_element_state(element: Element.ElementKind):
	if _current_state == null or _current_state == element:
		_current_state = element
		return

	var reaction = _resolve_reaction(element)
	return reaction

func _resolve_reaction(element: Element.ElementKind) -> ReactionData:
	var reaction = get_reaction(_current_state, element)
	assert(reaction != null, "Unknown reaction between '%s' and '%s'" % [get_element_name(_current_state), get_element_name(element)])
	
	var reaction_data = ReactionData.new()
	reaction_data.reaction = reaction
	reaction_data.elements.append_array([_current_state, element])
	_current_state = null
